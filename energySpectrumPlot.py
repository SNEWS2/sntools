...
#np.histogram(TValues, bins=100)
plt.hist(ePartValues, bins=25, histtype="step")
...
y,binEdges=np.histogram(ePartValues,bins=25)
bincenters = 0.5*(binEdges[1:]+binEdges[:-1])
p10 = np.poly1d(np.polyfit(bincenters, y, 7))
p.plot(bincenters, y, '-', bincenters, p10(bincenters), '-')
p.show()
