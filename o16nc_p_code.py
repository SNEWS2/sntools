...
e_thr = 14  # approximate energy threshold of proton knock-out reaction with no gamma emission (MeV)
e_thr_g = e_thr + 6.32  # energy threshold of proton knock-out reaction with gamma emission (MeV)
...
class Channel(BaseChannel):
    def generate_event(self, eNu, dirx, diry, dirz):
        ...
        eP, dirxP, diryP, dirzP = self.get_proton_kinematics(eNu, eE, dirx, diry, dirz)
    ...
    def get_proton_kinematics(self, eNu, eE, dirx, diry, dirz):
        '''
        eN = mP + eNu - eE  # proton energy

        # calculate 3-momentum of proton ...
        pE = sqrt(eE**2 - mE**2)
        pN_x = - dirx * pE
        pN_y = - diry * pE
        pN_z = eNu - dirz * pE
        # ... and normalize it to get unit vector of proton direction
        pN = sqrt(pN_x**2 + pN_y**2 + pN_z**2)''' #code used in ibd code
        return (eP, pP_x / pP, pP_y / pP, pP_z / pP)
    ...
    def dSigma_dE(self, eNu, eE):
        ...
        sigma = fit('proton')(eNu)  # cross-section at eNu from the interp1d fit of Suzuki et al. 2018 data
